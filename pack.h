...
#define NTHREADS 8
#define NC 16 * NTHREADS * 1
#define MC 6 * NTHREADS * 80
#define KC 200
...
void pack_blockB(const float* B, float* packed_B, const int nc, 
                const int NR, const int kc, const int N) {
    for(int Bb_row = 0; Bb_row < nc; Bb_row += NR) {
        int nr = min(NR, nc - Bb_row);
        pack_panelB(&B[Bb_row], &packed_B[Bb_row], nr, kc, N);
    }
